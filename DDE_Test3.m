...
MVv0t=zeros(nc.n,1);
MVv0t(1)=v0t;
MVv0t=inv(nc.M*V)*MVv0t;
%% MVE without lag                          Energy for cutting - Wc only for last element of drillstring (n)
E=zeros(nc.n,1);
...
x2 = x(nc.n+1:2*nc.n);
x1dot = x2;
x2dot = MVv0t + MVE.*xlag - D*x1 -  nc.C*x2;                       %Lag is zero %%Missing VGB - GETTING ERROR
xdot = [x1dot; x2dot];
end
...
%% The differential equations:
    ...
    x1dot_nd = x2_nd;
    x2dot_nd = MVv0t + MVE.*x1_nd - D*x1_nd -  nc.C*x2_nd;                       %Lag is zero %%Missing VGB - GETTING ERROR
